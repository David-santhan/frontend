...
let navigate = useNavigate();
    ...
    let getUserList=async()=>{
        ...
        let JSONData= await fetch("https://hrbackend-e58m.onrender.com/userDetailsHome",reqOption)
      ...
      let deleteUserDetails = async (id) => {
        ...
        if (confirmation) {
            ...
            try {
                let JSONData = await fetch(`https://hrbackend-e58m.onrender.com/deleteUser/${id}`, reqOption);
                let JSOData = await JSONData.json();
        ...
        <Col>
      {
        showUserData&&showUserData.map((item)=>{
          ...
          <Toast.Body style={{textAlign:"start"}}>
            <center>            <img style={{width:"200px"}}src={`https://hrbackend-e58m.onrender.com/${item.ProfilePic}`} ></img></center> <hr></hr>

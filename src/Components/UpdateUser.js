...
function UpdateUser() {
    ...
    useEffect(() => {
        axios.get(`https://ornnovabackend.onrender.com/getUserdatatoUpdate/${id}`)
            .then(res => {
                setUserValue({
                    ...userValue,
    ...
    const handleSubmit = async (e) => {
      ...
      try {
          const response = await axios.put(`https://ornnovabackend.onrender.com/updateUser/${id}`, updatedUserData);
    ...
    const gettingUsersData = async () => {
        try {
            const response = await fetch("https://ornnovabackend.onrender.com/getUserDataToADDtoTeam", {
                method: "GET"
            });
    ...
    return (
        <div>
            ...
            <Form onSubmit={handleSubmit} style={{ backgroundColor: "aliceblue", padding: "30px", borderRadius: "20px", margin: "15px" }}>
                <Image src={`https://ornnovabackend.onrender.com/${userValue.profile}`} style={{ width: "100px", borderRadius: "100px" }} alt="Profile" />
